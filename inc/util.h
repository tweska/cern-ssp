...
template <typename T>
b8 checkArrayAvg(const usize n, const usize k, const T *observed, const T *expected, const f64 maxError = 0.000000000001, const isize maxReports = 25, std::ostream &output = std::cerr) {
...
template <typename T>
void arrayMaxError(T *maxError, T *oValue, T *eValue, const usize n, const T *observed, const T *expected) {
    assert(n > 0);
    *maxError = fabsl((observed[0] - expected[0]) / expected[0]);
    *oValue = observed[0];
    *eValue = expected[0];
    for (usize i = 1; i < n; ++i) {
        if (const f64 error = fabsl((observed[i] - expected[i]) / expected[i]); error > *maxError) {
            *maxError = error;
            *oValue = observed[i];
            *eValue = expected[i];
        }
